...
export class LeyerService {
  ...
  public getAll(page?: number | undefined): Observable<any> {
    return this.http
      .get(environment.api + "layers/list", { page: page })
      .map(res => {
        return res.json();
      });
  }
  ...
  public save(menu: Layer): Observable<any> {
    return this.http.post(environment.api + "layers/save", menu);
  }

  public delete(menu: Layer): Observable<any> {
    return this.http.delete(environment.api + "layers/delete/" + menu.id);
  }
}
